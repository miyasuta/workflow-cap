...
export class UsersRequestBuilder extends RequestBuilder<Users> {
   ...
   * @param id Key property. See [[Users.id]].
   * @returns A request builder for creating requests to retrieve one `Users` entity based on its keys.
   */
  getByKey(id: string): GetByKeyRequestBuilderV4<Users> {
    return new GetByKeyRequestBuilderV4(Users, { id: id });
  }
   ...
   */
  delete(id: string): DeleteRequestBuilderV4<Users>;
  /**
   * Returns a request builder for deleting an entity of type `Users`.
  ...
  delete(idOrEntity: any): DeleteRequestBuilderV4<Users> {
    return new DeleteRequestBuilderV4(Users, idOrEntity instanceof Users ? idOrEntity : { id: idOrEntity! });
  }
}
